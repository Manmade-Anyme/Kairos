...
- **MANM-103 OI Flow remediation:** made consensus current-first and direction-matched, removed PCR from trade gates, validated Greek aggregates, corrected Vega exposure normalization, capped GO on effective OI vetoes, and emitted deduplicated OI events below the score-silencing threshold. Reject incomplete Greek rows, report stale observations before generic invalid data, ignore stale samples in historical Vega-trap counts, and fingerprint semantic OI categories without diagnostic vote counts. Documented the explicit policy that Short Covering and Long Unwinding remain eligible (overriding the legacy buildup-only checklist).
